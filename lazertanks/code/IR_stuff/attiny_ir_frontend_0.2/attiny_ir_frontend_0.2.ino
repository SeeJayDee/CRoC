 ...
 * 
 * Make sure you select Adafruit Trinket 8MHz, and upload only
...
void loop() {
  if (irrecv.decode(&receiveValue)) {
    ...
    message = byte((receiveValue.value)&mask);
    TinyWireM.beginTransmission(MCU_ADDRESS); // initiate i2c transmission to mcu
    TinyWireM.send(message); // subtract padding, constrain to 8 bits & send
  ...
  if (!digitalRead(CONTROL_PIN)){
    ...
    transmitValue = TinyWireM.receive() + ~mask; // receive byte & add padding (4 leading 1s)
